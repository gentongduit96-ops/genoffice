...
import {
  allowDocWrite as allowDocsDocWrite,
  rememberDiskState as rememberDocsDiskState,
...
export function registerManuscriberIpc(): void {
  ipcMain.removeHandler('ai:analyze-media')
  ipcMain.handle(
    'ai:analyze-media',
    async (_event, op: { mediaUrls: string[]; requirements: string }) => {
      return analyzeMediaTool(
        ...
        { notLoggedInError: 'Genspark sign-in required' },
      )
    },
  )

  ipcMain.removeHandler('manuscriber:pick-pdf')
  ipcMain.handle('manuscriber:pick-pdf', async (event) => {
    const parent = BrowserWindow.fromWebContents(event.sender) ?? mainWindow ?? null
    const options: OpenDialogOptions = {
      title: 'Pilih File PDF Manuskrip',
      properties: ['openFile'],
      filters: [
        { name: 'PDF & Proyek Manuscriber (*.pdf, *.manus)', extensions: ['pdf', 'manus', 'manuscriber', 'mnsproj'] },
  ...
  ipcMain.removeHandler('manuscriber:save-project-as')
  ipcMain.handle(
    'manuscriber:save-project-as',
    async (event, defaultName: string, data: ArrayBuffer, sourcePath?: string | null) => {
      ...
      const baseName = defaultName.replace(/\.(docx|pdf|manus|manuscriber|mnsproj)$/i, '')
      const result = await saveDialog(event, {
        title: 'Simpan Proyek Manuscriber',
        defaultPath: saveAsSuggestion(
          typeof sourcePath === 'string' ? sourcePath : null,
          `${baseName}.manus`,
        ),
        filters: [{ name: 'Proyek Manuscriber (*.manus)', extensions: ['manus'] }],
      })
      ...
      try {
        ...
        try {
          allowDocsDocWrite(event.sender.id, result.filePath)
        } catch {
          /* fallback */
        }
        await rememberDiskState(event.sender.id, result.filePath, bytes)
        try {
          await rememberDocsDiskState(event.sender.id, result.filePath, bytes)
        } catch {
          /* fallback */
        }
  ...
  ipcMain.removeHandler('manuscriber:read-pdf')
  ipcMain.handle('manuscriber:read-pdf', async (_event, filePath: string) => {
    try {
  ...
  ipcMain.removeHandler('manuscriber:consume-pending-pdf')
  ipcMain.handle('manuscriber:consume-pending-pdf', async (event) => {
    const queued = pendingWindowOpens.get(event.sender.id)
    if (queued && queued.toLowerCase().endsWith('.pdf')) {
      pendingWindowOpens.delete(event.sender.id)
      try {
        const data = readFileSync(queued)
        return {
          ok: true,
          filePath: queued,
...
export function registerDocsIpc(): void {
  ...
  ipcMain.handle('docs:open', async (event) => {
    const result = await openDialog(event, {
      title: tm('dlgOpenDoc'),
      filters: [
        { name: 'Dokumen Word & Proyek Manuscriber (*.docx, *.manus)', extensions: ['docx', 'manus', 'manuscriber', 'mnsproj'] },
  ...
  ipcMain.handle('docs:consume-pending-open', (event) => {
    ...
    const queued = pendingWindowOpens.get(event.sender.id)
    if (queued) {
      if (queued.toLowerCase().endsWith('.pdf')) {
        return null
      }
      pendingWindowOpens.delete(event.sender.id)
...
export function setupSpellChecker(ses: Electron.Session = session.defaultSession): void {
  try {
    ...
    console.log('[SpellChecker] Multi-language spellchecker active:', ses.getSpellCheckerLanguages())
  } catch (err) {
...
export function createDocsView(openPath?: string): WebContentsView {
  ...
  void view.webContents.loadURL(rendererUrl(runtime.rendererUrl, 'manuscriber', { mode: 'tab' }))
