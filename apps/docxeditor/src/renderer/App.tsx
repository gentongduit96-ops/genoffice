...
import { PageAnalysisProgress } from './components/PageAnalysisProgress'
import { executePageAnalysis, type AnalysisProgressInfo } from './services/page-analyzer'
import {
  appendEndnotesBlock,
...
export function App() {
  ...
  useEffect(() => {
    const unsub = window.desktop?.onSpellingSuggestions?.((data) => {
  ...
  useEffect(() => {
    document.title = doc ? doc.fileName : 'Manuscriber'
  }, [doc])
  ...
  const manuscriberBridgeRef = useRef<ManuscriberBridge | null>(null)
  ...
  const [promptManagerCategory, setPromptManagerCategory] = useState<string>('all')
  const [showPageAnalysisModal, setShowPageAnalysisModal] = useState(false)
  const [pageAnalysisProgress, setPageAnalysisProgress] = useState<AnalysisProgressInfo | null>(null)
  const pageAnalysisAbortRef = useRef<AbortController | null>(null)
  ...
  const handleSaveProject = useCallback(
    () => saveProjectImpl(fileCtxRef.current, false),
    [],
  )

  const handleSaveProjectAs = useCallback(
    () => saveProjectImpl(fileCtxRef.current, true),
    [],
  )

  const handleExportDocx = useCallback(
    () => exportDocxImpl(fileCtxRef.current),
    [],
  )

  const handleOpenProjectFile = useCallback(
    async (data: ArrayBuffer, name: string) => {
      await loadFileImpl(fileCtxRef.current, { path: name, name, data, hash: '' })
    },
    [],
  )
  ...
  const handleStartPageAnalysis = useCallback(
    (options: {
      ...
      if (pageAnalysisAbortRef.current) {
        pageAnalysisAbortRef.current.abort()
      }
      const abortController = new AbortController()
      pageAnalysisAbortRef.current = abortController

      setPageAnalysisProgress({
        currentPage: 1,
        totalPages: options.selectedPageIndices.length,
        pageNumber: (options.selectedPageIndices[0] ?? 0) + 1,
      ...
      executePageAnalysis({
        editor,
        totalPagesHint: pageInfo.total,
        selectedPageIndices: options.selectedPageIndices,
        ...
        setShowComments,
        onProgress: (prog) => {
          setPageAnalysisProgress(prog)
          if (prog.status === 'completed') {
            setStatus(
              `✨ Analisis halaman selesai: ${prog.commentsAddedCount} komentar disematkan.`,
            )
          } else if (prog.status === 'cancelled') {
            setStatus('Analisis halaman dihentikan.')
          }
        },
        signal: abortController.signal,
      }).catch((err) => {
        console.error('Page analysis error:', err)
        setStatus('Terjadi kesalahan saat menganalisis halaman.')
      })
    },
    [editor, comments, setComments, setCommentsDirty, setShowComments, setStatus],
  )
  ...
  const ribbonActions = useStableCallbacks({
    ...
    onOpenPromptManager: (category: string = 'all') => {
      setPromptManagerCategory(category)
    ...
    onPageAnalysis: () => setShowPageAnalysisModal(true),
...
.editor-scroll .doc-page.measuring-columns { column-count: auto; width: ${colFlow.colWidthPx + twipsToPx(canvasSection?.marginLeft ?? section?.marginLeft ?? 0) + twipsToPx(canvasSection?.marginRight ?? section?.marginRight ?? 0)}px; }`}</style>
      ...
      <div className="app-main">
        <PdfViewerPanel
          editor={editor}
          bridgeRef={manuscriberBridgeRef}
          initialPdf={projectPdf}
          onSaveProject={handleSaveProject}
          onSaveProjectAs={handleSaveProjectAs}
      ...
      {doc && showPageAnalysisModal && editor && (
        <PageAnalysisModal
          editor={editor}
          pageInfo={pageInfo}
          onClose={() => setShowPageAnalysisModal(false)}
          onStart={handleStartPageAnalysis}
        />
      )}

      {pageAnalysisProgress && (
        <PageAnalysisProgress
          progress={pageAnalysisProgress}
          onCancel={() => {
            pageAnalysisAbortRef.current?.abort()
            setPageAnalysisProgress((prev) =>
              prev ? { ...prev, status: 'cancelled' } : null,
            )
          }}
          onDismiss={() => setPageAnalysisProgress(null)}
        />
      )}
      ...
      {showPromptManager && (
        <PromptManagerModal
          initialCategory={promptManagerCategory}
          onClose={() => setShowPromptManager(false)}
          onExecutePrompt={(instruction) => {
            setShowAi(true)
            setAiPreset({ text: instruction, nonce: Date.now(), autoRun: true })
          }}
        />
      )}
    </div>
