...
export function EditorContextMenu({
  ...
  const replaceMisspelled = (replacement: string) => {
    ...
    const range = wordRangeAtCaret(editor)
    if (range) {
  ...
  return (
    ...
    >
      {spellingSuggestions && spellingSuggestions.misspelledWord && (
        <>
          {spellingSuggestions.suggestions.length > 0 ? (
            spellingSuggestions.suggestions.slice(0, 5).map((sugg) => (
              <button
                key={sugg}
                className="ctx-item"
                style={{ fontWeight: 600, color: 'var(--primary-color, #1a73e8)' }}
                onClick={run(() => replaceMisspelled(sugg))}
              >
                <span className="ctx-label">{sugg}</span>
              </button>
            ))
          ) : (
            <div className="ctx-item" style={{ opacity: 0.6, cursor: 'default', pointerEvents: 'none' }}>
              <span className="ctx-label">Tidak ada saran ejaan</span>
            </div>
          )}
          <button
            className="ctx-item"
            onClick={run(() => {
              void window.desktop?.addWordToDictionary?.(spellingSuggestions.misspelledWord)
            })}
          >
            <span className="ctx-label">Tambahkan ke Kamus</span>
          </button>
