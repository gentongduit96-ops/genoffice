...
interface RibbonProps {
  ...
  onOpenPromptManager?: (category?: string) => void
...
export interface StyleGalleryPreset {
  key: string
  label: string
  labelKey?: StringKey
  className: string
  previewText?: string
  tooltip?: string
  category: 'standard' | 'turats' | 'academic'
}

const STYLE_GALLERY: readonly StyleGalleryPreset[] = [
  // Standar
  { key: 'p', label: 'Normal', labelKey: 'ribbonStyleNormal', className: 'style-normal', previewText: 'AaBb', category: 'standard' },
  { key: 'h1', label: 'Judul 1', labelKey: 'ribbonStyleHeading1', className: 'style-h1', previewText: 'AaBb', category: 'standard', tooltip: 'Judul 1: Bab Utama (Heading 1)' },
  { key: 'h2', label: 'Judul 2', labelKey: 'ribbonStyleHeading2', className: 'style-h2', previewText: 'AaBb', category: 'standard', tooltip: 'Judul 2: Fasal / Bagian (Heading 2)' },
  { key: 'h3', label: 'Judul 3', labelKey: 'ribbonStyleHeading3', className: 'style-h3', previewText: 'AaBb', category: 'standard', tooltip: 'Judul 3: Sub-Fasal / Mabhats (Heading 3)' },

  // Format Turats & Manuskrip
  { key: 'turats:matan', label: 'Matan', className: 'style-matan', previewText: 'متن', category: 'turats', tooltip: 'Matan: Teks Pokok Kitab (Arab Amiri 14pt, Tebal, RTL, 1.5 Spasi)' },
  { key: 'turats:syarah', label: 'Syarah', className: 'style-syarah', previewText: 'شرح', category: 'turats', tooltip: 'Syarah: Penjelasan / Komentar (Arab Amiri 12pt, Indent 1cm, Justify)' },
  { key: 'turats:hasyiah', label: 'Hasyiah', className: 'style-hasyiah', previewText: 'حاشية', category: 'turats', tooltip: 'Hasyiah: Catatan Pinggir / Glosarium (Amiri 10.5pt, Indent Kanan-Kiri)' },
  { key: 'turats:nadzom', label: 'Nadzom', className: 'style-nadzom', previewText: 'نظم', category: 'turats', tooltip: 'Nadzom: Bait Syair / Puisi Klasik (Tengah, 13pt)' },
  { key: 'turats:quran', label: "Ayat Qur'an", className: 'style-quran', previewText: '﴿ ﴾', category: 'turats', tooltip: "Ayat Al-Qur'an: Bungkus tanda ﴿ ﴾ (Amiri 14pt Tebal)" },
  { key: 'turats:hadith', label: 'Hadits', className: 'style-hadith', previewText: '« »', category: 'turats', tooltip: 'Hadits Nabawi: Bungkus tanda « » (Amiri 13pt Miring/Tebal)' },

  // Format Akademik
  { key: 'academic:quote', label: 'Kutipan', className: 'style-quote', previewText: '“ ”', category: 'academic', tooltip: 'Kutipan Blok: Nukilan Ilmiah (Indent 1.25cm, 11pt, 1.15 Spasi)' },
]
...
function RibbonInner({
  ...
  const [allPresets, setAllPresets] = useState<PromptPreset[]>(() => getPresetsByCategory('all'))

  useEffect(() => {
    return subscribePromptPresets((updated) => {
      setAllPresets(updated)
    })
  }, [])

  const summarizePresets = allPresets.filter((p) => p.category.toLowerCase() === 'ringkasan')
  const polishPresets = allPresets.filter((p) => p.category.toLowerCase() === 'poles')
  const formatPresets = allPresets.filter((p) => p.category.toLowerCase() === 'format')
      ...
      : activeCharStyleId
        ...
        : presetActive('bold')
          ...
          : presetActive('italic')
            ...
            : fs.align === 'center' &&
                (fs.fontFamily?.toLowerCase().includes('amiri') ||
                  fs.fontFamily?.toLowerCase().includes('arabic'))
              ? 'turats:nadzom'
              : fs.fontSizePt &&
                  fs.fontSizePt >= 13.5 &&
                  fs.bold &&
  ...
  const applyStyle = (key: string) => {
    ...
    if (key === 'turats:matan') {
      ...
      setParaAttrs(editor, { align: 'justify', lineSpacing: 1.5, indentFirstLine: 0, indentLeft: 0, indentRight: 0 })
      chain().setNode('docParagraph').setMark('docTextStyle', { font: 'Amiri', fontAscii: 'Amiri', sizeHalfPoints: 28 }).setMark('bold').unsetMark('italic').run()
      return
    }

    if (key === 'turats:syarah') {
      if (sub) return
      setParagraphDirection(editor, 'rtl')
      setParaAttrs(editor, { align: 'justify', lineSpacing: 1.5, indentFirstLine: 480, indentLeft: 0, indentRight: 0 })
      chain().setNode('docParagraph').setMark('docTextStyle', { font: 'Amiri', fontAscii: 'Amiri', sizeHalfPoints: 24 }).unsetMark('bold').unsetMark('italic').run()
      return
    }

    if (key === 'turats:hasyiah') {
      if (sub) return
      setParagraphDirection(editor, 'rtl')
      setParaAttrs(editor, { align: 'justify', lineSpacing: 1.15, indentFirstLine: 0, indentLeft: 360, indentRight: 360 })
      chain().setNode('docParagraph').setMark('docTextStyle', { font: 'Amiri', fontAscii: 'Amiri', sizeHalfPoints: 21 }).unsetMark('bold').run()
      return
    }

    if (key === 'turats:nadzom') {
      if (sub) return
      setParagraphDirection(editor, 'rtl')
      setParaAttrs(editor, { align: 'center', lineSpacing: 1.3, indentFirstLine: 0, indentLeft: 0, indentRight: 0 })
      chain().setNode('docParagraph').setMark('docTextStyle', { font: 'Amiri', fontAscii: 'Amiri', sizeHalfPoints: 26 }).setMark('bold').run()
      return
    }
    ...
    if (key === 'turats:quran') {
      ...
      chain().setMark('docTextStyle', { font: 'Amiri', fontAscii: 'Amiri', sizeHalfPoints: 28 }).setMark('bold').run()
      return
    }

    if (key === 'turats:hadith') {
      const { from, to, empty } = editor.state.selection
      if (!empty) {
        const text = editor.state.doc.textBetween(from, to, ' ')
        if (!text.startsWith('«') && !text.endsWith('»')) {
      ...
      chain().setMark('docTextStyle', { font: 'Amiri', fontAscii: 'Amiri', sizeHalfPoints: 26 }).setMark('italic').run()
      return
    }

    if (key === 'academic:quote') {
      if (sub) return
      setParaAttrs(editor, { align: 'justify', lineSpacing: 1.15, indentFirstLine: 0, indentLeft: 720, indentRight: 720 })
      chain().setNode('docParagraph').setMark('docTextStyle', { sizeHalfPoints: 22 }).run()
      return
    }
  ...
  const renderStyleCards = (inMenu: boolean) => {
    ...
    const renderCard = (s: StyleGalleryPreset) => {
      const label = s.labelKey ? t(s.labelKey) : s.label
      const preview = s.previewText || t('ribbonStylePreview')
      return (
        <button
          key={s.key}
          className={`style-card ${activeStyleKey === s.key ? 'active' : ''}`}
          disabled={!canEdit || (!!sub && s.category === 'standard' && s.key !== 'p')}
          data-tip={s.tooltip || label}
          onClick={() => apply(s.key)}
        >
          <span className={`style-card-preview ${s.className}`}>{preview}</span>
    ...
    if (inMenu) {
      const standardStyles = STYLE_GALLERY.filter((s) => s.category === 'standard')
      const turatsStyles = STYLE_GALLERY.filter((s) => s.category === 'turats')
      const academicStyles = STYLE_GALLERY.filter((s) => s.category === 'academic')

      return (
        <div className="style-gallery-menu-sections">
          <div className="style-gallery-section">
            <div className="style-gallery-section-title">Gaya Standar & Judul</div>
            <div className="style-gallery-grid">{standardStyles.map(renderCard)}</div>
          </div>

          <div className="style-gallery-section">
            <div className="style-gallery-section-title">Format Turats & Manuskrip</div>
            <div className="style-gallery-grid">{turatsStyles.map(renderCard)}</div>
          </div>

          <div className="style-gallery-section">
            <div className="style-gallery-section-title">Format Akademik & Penekanan</div>
            <div className="style-gallery-grid">
              {academicStyles.map(renderCard)}
          ...
          <div className="file-tab-wrap">
            ...
            {dropdown === 'file' && (
              <div data-rb-panel="" className="file-menu">
                ...
                >
                  Simpan Dokumen Sebagai... (.docx) <span className="file-menu-key">Ctrl+Shift+S</span>
                </button>
                {onSaveProject && (
                  <>
                ...
                <div className="ribbon-group">
                  <div className="ribbon-group-items">
                    <button
                      ...
                      data-tip="Simpan Proyek Manuscriber (.manus)"
                    ...
                    >
                      <span className="rb-big-icon" style={{ color: 'var(--colorBrandForeground1, #3b82f6)' }}>
                        <IconSave size={24} />
                      </span>
