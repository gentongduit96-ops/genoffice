...
} from './manuscriber/project-io'
import type { ManuscriberBridge } from './manuscriber/PdfViewerPanel'
...
export interface FileActionContext {
  ...
  manuscriberBridgeRef?: React.MutableRefObject<ManuscriberBridge | null>
  setProjectPdf?: (val: { buffer: ArrayBuffer; name: string; meta?: ManusProjectMetadata['pdf'] } | null) => void
...
export async function loadFile(
  ...
  try {
    let docxRawBytes: ArrayBuffer = result.data
    let pdfInitialData: {
      buffer: ArrayBuffer
      name: string
    ...
    if (pdfInitialData) {
      ctx.setProjectPdf?.(pdfInitialData)
      if (ctx.manuscriberBridgeRef?.current) {
        void ctx.manuscriberBridgeRef.current.loadPdfFromBuffer(
          pdfInitialData.buffer,
          pdfInitialData.name,
          pdfInitialData.meta,
        )
      }
...
export function saveProject(
  ...
  return runSerializedSave(
    () => saveOnce(ctx, saveAs, false, undefined, true),
    () => false,
  )
}
...
export function exportDocx(
  ...
  return runSerializedSave(
    () => saveOnce(ctx, true, false, undefined, false, true),
    () => false,
  )
}
...
async function saveOnce(
  ...
  try {
    ...
    const isProject =
      (isManusProjectFile(savedPath || '') || isSaveProject === true) && !isExportDocx
    const pdfState = ctx.manuscriberBridgeRef?.current?.getPdfExportState()

    let buffer: ArrayBuffer
    if (isProject && pdfState) {
      const packed = await packManusProject({
        docxBytes: bytes,
        pdfBytes: pdfState.pdfBuffer,
        metadata: {
          version: '1.0.0',
          projectType: 'manuscriber',
          name: (doc.fileName || 'Proyek').replace(
            /\.(manus|manuscriber|mnsproj|docx)$/i,
            '',
          ),
          createdAt: new Date().toISOString(),
          updatedAt: new Date().toISOString(),
          pdf: {
            originalName: pdfState.fileName,
            totalPages: pdfState.totalPages,
            lastActivePage: pdfState.lastActivePage,
            lastZoom: pdfState.lastZoom,
            pageStatuses: pdfState.pageStatuses,
            pageErrors: pdfState.pageErrors,
          },
        },
      })
    ...
    if (
      saveAs ||
      !savedPath ||
      (isProject && !isManusProjectFile(savedPath))
    ) {
      ...
      const autoName =
        !doc.filePath && doc.fileName === t('appUntitledDocx')
          ...
          : null
      const baseSuggestName = newDocName ?? autoName ?? doc.fileName
      const cleanBase = (baseSuggestName || 'Dokumen').replace(
        /\.(docx|pdf|manus|manuscriber|mnsproj)$/i,
        '',
      )
      const suggestName = isProject ? `${cleanBase}.manus` : `${cleanBase}.docx`
      ...
      if (isProject && (!auto || saveAs || isSaveProject)) {
        result = await window.desktop.saveManusProjectAs(
          suggestName,
          buffer,
          doc.filePath,
        )
      } else if (saveAs) {
        result = await window.desktop.saveDocxAs(
          suggestName,
          buffer,
          doc.filePath,
        )
      } else {
        result = await window.desktop.saveDocxNew(suggestName, buffer)
      }
